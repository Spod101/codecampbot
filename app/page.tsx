...
// ── Sidebar ───────────────────────────────────────────────────────────────────
function Sidebar({ activeTab, activeChapter, chapters, onSwitch, onShowChapter }: {
  activeTab: TabId
        ...
        {NAV_SECTIONS.map(section => (
          <div key={section.label} style={{ marginBottom:'24px' }}>
            ...
            {section.items.map(item => {
              const isActive = activeTab === item.id && !activeChapter
              return (
                <button
                  key={item.id}
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:C.bg, fontFamily:"'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ flex:1, marginLeft:'280px', display:'flex', flexDirection:'column', minHeight:'100vh' }}>
        ...
        <main style={{ flex:1, padding:'32px', overflowX:'hidden' }}>
          {activeChapter ? (
            <ChapterDetailPanel chapterId={activeChapter} chapters={chapters} onBack={() => setActiveChapter(null)} />
